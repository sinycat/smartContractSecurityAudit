...
import type { ChainContractInfo, ContractBasicInfo, ContractFile } from '@/types/blockchain';

export async function checkContractOnChains(address: string): Promise<ChainContractInfo> {
  const result: ChainContractInfo = {};
      ...
      try {
        ...
        try {
          ...
          if (implementationResult.address) {
            ...
            const implContract = new ethers.Contract(implementationResult.address, [
              'function name() view returns (string)',
              'function symbol() view returns (string)',
              'function decimals() view returns (uint8)',
              'function totalSupply() view returns (uint256)',
...
async function fetchContractLabels(address: string, chain: string): Promise<{labels?: string[], projectName?: string}> {
  return KNOWN_CONTRACTS[address.toLowerCase()] || {};
}

export async function getImplementationAddress(proxyAddress: string, chain: string): Promise<{address: string | null, type: string | null}> {
  try {
    ...
    for (const method of ['implementation', 'getImplementation', 'masterCopy', 'getProxyImplementation']) {
      try {
